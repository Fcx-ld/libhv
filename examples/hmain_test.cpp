...
/*
1.static关键字可以用于类中声明静态成员变量。这意味着该变量是类的所有实例共享的，而不是每个实例拥有自己的副本。 
2.static关键字也可以用于声明静态成员函数。静态成员函数可以通过类名直接调用，不需要类的实例。
3.在函数内部，static关键字可以用于定义一个局部静态变量，该变量的生命周期贯穿整个程序，而不是仅限于函数调用的持续时间。
4.修饰普通变量，修改变量的存储区域和生命周期，使变量存储在静态区，在 main函数运行前就分配了空间，如果有初始值就用初始值初始化它，如果没有初始值系统用默认值初始化它。
5.修饰普通函数，表明函数的作用范围，仅在定义该函数的文件内才能使用。在多人开发项目时，为了防止与他人命令函数重名，可以将函数定位为static
*/ 
